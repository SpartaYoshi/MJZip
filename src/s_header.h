...
struct s_Info {
   ...
   char FileType;			 // ='r' (regular file), ='d' (directory)
					 // ='l' (symbolic link), ='o' (other type)
   mode_t Permissions;			 // Permissions in octal for User, Group and Others: Read, Write, Execute.
   int childIndex[NUM_HEADERS];		 // If the file is directory, here are stored its children file's indexes. Valid
					 // arrays are stored until the -1 index.
};
