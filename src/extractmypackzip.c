...
int main(int argc, char * argv[])
{
	...
	else if(resultado == 1)
	{
		...
		sprintf(mensaje, "Número de registro (índice) erroneo.\n");
	else if(resultado == 3)
		sprintf(mensaje, "No se puede crear el directorio indicado en el registro (index) \'%d\'.\n", atoi(argv[2]));
	else if(resultado == 4)
		sprintf(mensaje, "El archivo a exportar ya existe.\n");
	else if(resultado == 5)
		sprintf(mensaje, "Error al intentar descomprimir fichero regular.\n");
	else
